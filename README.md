...
### Начало работы
Если нет торгового токена, то получите его в настройках профиля Т-Инвестиции [здесь >>>](https://www.tinkoff.ru/invest/settings/)

Установите токен в защищенное системное хранилище через командную строку. Вместо **<Токен>** вставьте полученный токен:

**Linux:**
```bash
python3 -c "from TinvestPy import TinvestPy; tp_provider = TinvestPy('<Токен>')"
 ```
**Windows:**
```cmd
python -c "from TinvestPy import TinvestPy; tp_provider = TinvestPy('<Токен>')"
```
