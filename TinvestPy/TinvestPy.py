...
class TinvestPy:
    ...
    def __init__(self, token=None, demo=False):
        """Инициализация
        ...
        self.bonds: dict[str, instruments_pb2.BondsResponse] = {}  # Информация об облигациях
        self.currencies: dict[str, instruments_pb2.CurrencyResponse] = {}  # Информация о валютах
        self.futures: dict[str, tuple[instruments_pb2.FuturesResponse, instruments_pb2.GetFuturesMarginResponse]] = {}  # Информация о фьючерсах
    ...
    def get_symbol_info(self, class_code, symbol, reload=False) -> instruments_pb2.Instrument | None:
        ...
        :param str class_code: : Код режима торгов
        :param str symbol: Тикер
        :param bool reload: Получить информацию с T-Invest
        :return: Значение из кэша/T-Invest или None, если тикер не найден
        """
        if reload or (class_code, symbol) not in self.symbols:  # Если нужно получить информацию с T-Invest или нет информации о тикере в справочнике
    ...
    def figi_to_symbol_info(self, figi) -> instruments_pb2.Instrument | None:
        ...
        if instrument is not None:  # Если значение найдено в справочнике
    ...
    @staticmethod
    def timeframe_to_tinvest_timeframe(tf) -> tuple[marketdata_pb2.CandleInterval.ValueType, bool]:
        """Перевод временнОго интервала во временной интервал T-Invest

        :param str tf: Временной интервал https://ru.wikipedia.org/wiki/Таймфрейм
        :return: Временной интервал T-Invest, внутридневной бар
        """
        if tf[0:1] == 'D':  # 1 день
    ...
    @staticmethod
    def tinvest_timeframe_to_timeframe(tf) -> tuple[str, timedelta]:
        """Перевод временнОго интервала T-Invest во временной интервал и максимальный период запроса

        :param marketdata_pb2.CandleInterval.ValueType tf: Временной интервал T-Invest
        :return: Временной интервал https://ru.wikipedia.org/wiki/Таймфрейм и максимальный период запроса
        """
        # Ограничения на максимальный период запроса https://developer.tbank.ru/invest/intro/intro/load_history
    ...
    @staticmethod
    def timeframe_to_tinvest_subscription_timeframe(tf) -> marketdata_pb2.SubscriptionInterval.ValueType:
        """Перевод временнОго интервала во временной интервал подписки T-Invest
    ...
    @staticmethod
    def tinvest_subscription_timeframe_to_timeframe(tf) -> str:
        """Перевод временнОго интервала подписки T-Invest во временной интервал

        :param marketdata_pb2.SubscriptionInterval.ValueType tf: Временной интервал T-Invest
    ...
    def price_to_tinvest_price(self, class_code, symbol, price) -> float:
        ...
        if si.instrument_kind == common_pb2.INSTRUMENT_TYPE_BOND:  # Для облигаций
            if si.figi in self.bonds.keys():  # Если уже получили информацию об облигации
                bonds_response = self.bonds[si.figi]  # то берем ее
